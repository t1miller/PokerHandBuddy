package com.poker.pokerhandbuddy.ui.main.deuceswild
...
class DeucesWildFragment : Fragment(), CardSelectionDialog.CardTouched {
    ...
    companion object {
        ...
        @JvmStatic
        fun newInstance() = DeucesWildFragment()
    }
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        viewModel.deucesResult.observe(viewLifecycleOwner, Observer { deuces ->
            CardUiUtils.highlightHeldCards(holdViews,deuces.hand,deuces.allHands[0].first)
            setExpectedValue(getHeldCards())
            handsEvaluatedText.text = getString(R.string.hands_evaluated, deuces.trials*32)
            timeText.text = getString(R.string.time, deuces.time)
            CardUiUtils.showCards(cardViews,deuces.hand)
        })
        viewModel.isDeucesRunning.observe(viewLifecycleOwner, Observer { deucesRunning ->
            if(!deucesRunning){
        ...
        viewModel.getBestHandDeuces(getVisibleCards())
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_deuces_wild, container, false)
        ...
        val deucesAdLayout = view.findViewById<ConstraintLayout>(R.id.deucesAdLayout)
        deucesAdLayout.setOnClickListener {
        ...
        expectedPayout.setOnClickListener {
            val expectedValue = viewModel.deucesResult.value?.allHands?.find { it.first.toMutableSet() == getHeldCards().toMutableSet() }?.second
            StatDialogUtils.showDialog(requireContext(),viewModel.deucesResult.value,getVisibleCards(),getHeldCards(), expectedValue)
        }
    ...
    private fun setExpectedValue(hand: List<Card>) {
        val expectedValue = viewModel.deucesResult.value?.allHands?.find { it.first.toMutableSet() == hand.toMutableSet() }?.second ?: 0.0
        expectedPayText.text = getString(R.string.expected_payout, expectedValue)
    }

    private fun setPayTableAndBet() {
        paytableText.text = getString(R.string.paytable, SettingsUtils.getPayoutTableDeucesString(context))
        betText.text = getString(R.string.bet, SettingsUtils.getBetDeuces(context))
    }
